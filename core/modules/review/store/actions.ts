...
const actions: ActionTree<ReviewState, RootState> = {
  async list (context, { productId, approved = true, start = 0, size = 50, entityType = 'review', sort = '', excludeFields = null, includeFields = null }) {
    ...
    //EventBus.$emit('notification-progress-start', i18n.t('List review ...'))
    const isReviewCreated = await ReviewsService.listReview(productId)
      //console.log('4+++++++++++')
    //console.log(isReviewCreated.result)
    //Promise.resolve(isReviewCreated.code === 200 ? isReviewCreated : null)

    return isReviewCreated.result
