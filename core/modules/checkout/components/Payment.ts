...
export const Payment = {
  ...
  created () {
    if (!this.payment.paymentMethod || this.notInMethods(this.payment.paymentMethod)) {
      this.payment.paymentMethod = this.paymentMethods.length > 0 ? this.paymentMethods[0].code : 'free_checkout'
    }
  },
  beforeMount () {
  ...
  methods: {
    sendDataToCheckout () {
      this.$store.state.cart.order_time = this.order_time;
          ...
          for (let i = 0; i < addresses.length; i++) {
            if (toString(addresses[i].id) === toString(id)) {
              this.payment = {
                ...
                paymentMethod: this.paymentMethods[0] ? this.paymentMethods[0].code : 'free_checkout'
              }
              //this.isFilled = true
