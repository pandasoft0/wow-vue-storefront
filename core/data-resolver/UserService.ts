...
const getMessages = async (idOrder: number) =>
  TaskQueue.execute({
    url: processLocalizedURLAddress(getApiEndpointUrl(config.users, 'messages_endpoint').replace('{{order}}', idOrder + '')),
    payload: {
      method: 'GET',
      mode: 'cors',
      headers
...
const addMessage = async (idOrder:number, idUser:number, user: string, date: string, message: string): Promise<any> =>
  TaskQueue.queue({
    url: processLocalizedURLAddress(getApiEndpointUrl(config.users, 'addmessage_endpoint')),
    payload: {
      method: 'POST',
      headers,
      mode: 'cors',
      body: JSON.stringify( {idOrder, idUser, user, date, message})
    },
  })
