<template>
  <form class="custom-options mb20">
    <div class="row">
      <div class="col-md-4 relative-cm-op-sel" v-for="option in product.custom_options" :key="('customOption_' + option.option_id)"  v-if="option.type === 'select'">
          <b v-html="option.title" />
          <select
            @change="optionChanged(option)"
            class="cl-secondary cl-secondary-op-sel"
            :name="('customOption_' + option.option_id)"
            :id="('customOption_' + option.option_id)"
            v-model="inputValues[('customOption_' + option.option_id)]"
          >
            <option v-for="opval in option.values" :selected="opval.sel === 1" :date-cnt="opval.quantity" :value="opval.option_type_id">{{opval.title}}</option>
          </select>
      </div>
    </div>

    <div v-for="(option,index) in product.custom_options" :key="('customOption_' + option.option_id)"  v-if="option.type === 'checkbox'">
    ...
    <div v-for="option in product.custom_options" :key="('customOption_' + option.option_id)" v-if="option.type !== 'select' && option.type !== 'checkbox'">
      <div class="custom-option mb15">
        <h4>{{ option.title }}</h4>
        <input
          class="
        ...
        <div v-if="option.type === 'radio' || option.type === 'drop_down'">
          <div class="m5 relative" v-for="opval in option.values" :key="opval.option_type_id">
            <input
              @change="optionChanged(option)"
              type="radio"
