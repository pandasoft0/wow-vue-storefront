...
export default {
  ...
  props: {
    ...
    heading: {
      type: String,
      required: false,
      default: i18n.t('Related services')
    }
  },
  components: {
    ProductListing
  ...
  methods: {
    async refreshList () {
      if(this.getCurrentProduct.related_products) {
        ...
        let relatedProductsQuery = prepareAllQueryFx(this.getCurrentProduct.related_products)
  ...
  computed: {
    ...
    getCurrentRelatedProducts () {
      //console.log(this.getProductRelated);
