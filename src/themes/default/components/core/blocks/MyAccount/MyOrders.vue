        ...
        <table class="brdr-1 brdr-cl-bg-secondary">
          ...
          <tbody>
            <tr class="brdr-top-1 brdr-cl-bg-secondary" v-for="order in ordersHistory" :key="order.entity_id">
              <td class="fs-medium-small lh25">
                #{{ order.entity_id }}
              </td>
              <td class="fs-medium-small lh25 hide-on-xs">
              ...
              <td class="fs-medium-small lh25 hide-on-xs">
                <a href="#" class="no-underline py10 px15 mb5 mr20 lnkbtn" @click.prevent="remakeOrder(skipGrouped(order.items))">{{ $t('Remake order') }}</a>
                    <router-link class="no-underline py10 px15 lnkbtn" :to="localizedRoute(`/my-account/orders/${order.entity_id}`)">
                      {{ $t('View order') }}
                    </router-link>
              </td>
            </tr>
