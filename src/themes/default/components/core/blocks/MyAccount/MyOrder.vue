  ...
  <div class="mb35" v-if="order">
    ...
    <div class="row fs16 mb20">
      <div class="col-xs-12 h4">
        ...
        <p class="mt35">
          <a href="#" class="underline lnkbtn" @click.prevent="remakeOrder(singleOrderItems)">{{ $t('Remake order') }}</a>
        </p>
      </div>
    </div>
    <div class="row fs16 mb35">
      <div class="col-xs-4 col-md-4">
        <chat
          :id-order="order.entity_id"
        />
      </div>
      <div class="col-xs-6 col-md-6 h4">
          ...
          <tbody>
            <tr class="brdr-top-1 brdr-cl-bg-secondary" v-for="item in singleOrderItems" :key="item.item_id">
              ...
              <td class="fs-medium lh25">
                <product-image :image="{src: itemThumbnail[item.item_id]}" />
              </td>
            </tr>
          </tbody>
          <tfoot>
...
import Chat from 'src/modules/chat/Chat.vue'
...
export default {
  ...
  mounted () {
    this.singleOrderItems.forEach(async item => {
      if (!this.itemThumbnail[item.item_id]) {
        const product = await this.getProduct({ options: { sku: item.item_id } })
        const thumbnail = productThumbnailPath(product)
        Vue.set(this.itemThumbnail, item.item_id, getThumbnailPath(thumbnail, 280, 280))
      }
    })
  }
}
